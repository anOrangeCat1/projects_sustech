    ...
    for p in neighbors([1, 1]):
        ...
        if kernal[i][j] == 255:
            if state == 0:
                ...
                state = 3
            elif state == 4 or state == 12 or state == 13:
                state = 20
            elif state == 11:
                state = 12
        else:
            if state == 0:
                ...
                state = 20
    flag = 0
    if state == 3 or state == 4 or state == 11 or state == 12 or state == 13:
        flag = 1
    elif state == 20:
        flag = 2
    return flag
...
def main():
    ...
    # note.write(str(paths))
    # note.close()
    # print(str(paths))
    # size = flags.shape
    # show(size, paths, 0.0001)
