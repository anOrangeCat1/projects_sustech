...
class MLP(nn.Module):
    """多层感知机MLP
    fully connected feedforward deep network
    1个输入层(input), 4个隐藏层(hide), 1个输出层(output)
    激活函数使用 ReLU(), 经测试, ReLU() 表现比 Sigmoid() 好很多
    具体结构如下所示
    """
...
def k_fold(k, my_net, loss, all_features, all_labels, num_epochs,
    ...
    for i in range(k):
        ...
        if i == 0:
            ...
            plt.ylabel("Loss")
            plt.title("Loss Figure of Fully Connected Feedforword Deep Network")
            plt.legend()
            plt.show()
...
# 注意，lr不可以过大
k, num_epochs, lr, weight_decay, batch_size = 5, 200, 0.0003, 0, 64
